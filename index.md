...
## Information Fusion in Manifolds
...
In this work we are interested in determining sparse architectures for control and estimation, for large-scale dynamical systems. The emphasis is on utility and privacy in these problems. Utility is defined as accuracy or performance that is below a user defined bound. Whereas, privacy is defined as accuracy or performance that is above a user defined bound. These problems are considered in the Kalman filtering framework, including ensemble and unscented variants, and $\mathcal{H}_2$ and $\mathcal{H}_\infty$ framework. The work is being funded by NSF and AFOSR.
